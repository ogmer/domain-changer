...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  if (msg?.type === 'performReplace') {
    ...
    performReplace(from, to, mode).then((result) => sendResponse({ ok: true, result })).catch((err) => sendResponse({ ok: false, error: String(err) }));
  ...
  for (const b of list) {
    try {
      ...
      if (matchFn(url.hostname)) {
        ...
        await new Promise((resolve) => chrome.bookmarks.update(b.id, { url: newUrl }, resolve));
